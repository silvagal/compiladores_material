\subtitle{\bf Aula 19 - Introdução}
\begin{frame}[t,plain]
    \titlepage
\end{frame}
...
\begin{frame}{Objetivos}
    ...
    \begin{itemize}
        ...
        instruções WAT.
        \item
        Discutir a reconstrução do controle de fluxo estruturado de
...
\begin{frame}[fragile]{WebAssembly como Alvo}
    ...
    \begin{itemize}
        ...
        \item
        \textbf{WebAssembly} (Wasm): especificação de máquina virtual com
        semântica formal (W3C)
        \item
        ...
        \begin{itemize}
            ...
            \item
            Semântica \textbf{precisa} e publicada
            \item
            Representação textual (WAT) \textbf{legível}
            \item
            Ferramentas: \texttt{wabt} (compilar) e
            \texttt{wasmtime} (executar)
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Por que WebAssembly?}
    ...
    \begin{itemize}
        ...
        \item
        Tipo único no nosso subconjunto: \texttt{i32}
        (inteiro de 32 bits)
        \item
        Todos os valores da IRT --- inteiros, ponteiros, endereços --- mapeiam
        para \texttt{i32}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Estrutura de um Módulo WAT}
    ...
    \begin{itemize}
        ...
        \item
        \texttt{(memory 4)} = 4 páginas de 64 KB = 256 KB de
        memória linear
        \item
        \texttt{$heap\_ptr} = ponteiro de alocação
        (\emph{bump-pointer})
        \item
        Funções \texttt{print} e
        \texttt{read\_int} importadas do ambiente
    \end{itemize}
\end{frame}
...
\begin{frame}{Máquina de Pilha}
    ...
    WebAssembly é uma \textbf{máquina de pilha}: instruções consomem do topo
    ...
    \begin{itemize}
        ...
        \item
        \(\sigma = [v_1, \ldots, v_n]\) --- pilha de operandos (topo à
        direita)
        \item
...
\begin{frame}[fragile]{Controle de Fluxo Estruturado}
    ...
    \begin{table}
        \begin{tabular}{l l l l}
            ...
            \hline
            Construtor & Comportamento de \texttt{br} \\
        \hline
        \texttt{(block \$l ...)} & Salta para o \textbf{fim} do
        bloco (\emph{break}) \\
        \texttt{(loop \$l ...)} & Salta para o \textbf{início}
        do laço (\emph{continue}) \\
        \texttt{(if (then ...) (else ...))} & Consome topo;
        executa ramo verdadeiro se \(\neq 0\) \\
        \texttt{br\_if \$l} & Consome topo; ramifica se
        \(\neq 0\) \\
        \hline
    \end{tabular}
\end{table}
...
\begin{frame}[fragile]{Padrão Canônico de While em WAT}

    \begin{lstlisting}
        (block $exit
        (loop $head
        ;; avaliac\~ao da condic\~ao de sa\'ida
        <condic\~ao>
        i32.eqz          ;; negac\~ao
        br_if $exit      ;; sai se condic\~ao falsa
        ;; corpo do laco
        <corpo>
        br $head         ;; volta ao in\'icio
        )
        )
    \end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{Alocador de Memória (Bump-Pointer)}
    ...
    \(\mathit{alloc}(n)\): aloca \(n\) palavras de 8 bytes, retorna endereço
    base.
\end{frame}

\begin{frame}{Tradução de Expressões
...
\begin{frame}[fragile]{Tabela de Operadores}
    ...
    \begin{table}
        \begin{tabular}{l l l l}
            ...
            IRT & WAT & IRT & WAT \\
            \hline
            ADD & \texttt{i32.add} & EQ &
            \texttt{i32.eq} \\
            SUB & \texttt{i32.sub} & NEQ &
            \texttt{i32.ne} \\
            MUL & \texttt{i32.mul} & LT &
            \texttt{i32.lt\_s} \\
            DIV & \texttt{i32.div\_s} & LE &
            \texttt{i32.le\_s} \\
            MOD & \texttt{i32.rem\_s} & GT &
            \texttt{i32.gt\_s} \\
            AND & \texttt{i32.and} & GE &
            \texttt{i32.ge\_s} \\
            OR & \texttt{i32.or} & & \\
            \hline
        \end{tabular}
...
\begin{frame}{Tradução de Comandos
...
\begin{frame}[fragile]{Sumário do Capítulo}
    ...
    \begin{itemize}
        ...
        \item
        Tradução de laços: \emph{linearizar} IRT, \emph{reconhecer} padrão,
...
\begin{frame}[fragile]{Próximos Passos}
    ...
    \begin{itemize}
        ...
        \texttt{v128} (SIMD)
        \item
        \textbf{Alocação de memória avançada}: GC, ponteiros managed
        \item
        \textbf{Otimizações específicas de Wasm}: tail calls, instruções SIMD
        \item
