...
\item
  Devido \`{a} avalia\c{c}\~{a}o lazy, \texttt{p2} só é avaliado se
  \texttt{p1} falhar
\end{itemize}
\end{frame}
...
\item
  Habilita nota\c{c}\~{a}o \texttt{do} para parsers mais
  legíveis
\end{itemize}
\end{frame}
...
\item
  \texttt{chainl}: associatividade \textbf{\`{a} esquerda}
  --- \texttt{1+2+3} →
  \texttt{(1+2)+3}
\end{itemize}
...
\item
  \texttt{chainr}: associatividade \textbf{\`{a} direita}
  --- \texttt{1+2+3} →
  \texttt{1+(2+3)}
\end{itemize}
...
\item
  \texttt{gen ops p} reconhece express\~{o}es com
  operadores de \texttt{ops} e operandos de
  \texttt{p}
\item
  Associatividade \`{a} esquerda via \texttt{chainl}
\end{itemize}
\end{frame}
...
\item
  \texttt{InfixL}: associatividade \`{a} esquerda;
  \texttt{InfixR}: \`{a} direita
\end{itemize}
\end{frame}
