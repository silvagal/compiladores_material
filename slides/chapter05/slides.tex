...
\hypertarget{gramuxe1ticas-livres-de-contexto}{%
\section{Gramáticas Livres de
Contexto}\label{gramuxe1ticas-livres-de-contexto}}
...
\tightlist
\item
  Deriva\c{c}\~{a}o Mais \`{a} Esquerda de \passthrough{\lstinline!(1+4)+2!}
\end{itemize}
...
\tightlist
\item
  Deriva\c{c}\~{a}o Mais \`{a} Direita de \passthrough{\lstinline!(1+4)+2!}
\end{itemize}
...
\item
  Para \passthrough{\lstinline!2 + 3 * 4!}:
\end{itemize}
\end{frame}
...
\tightlist
\item
  O \passthrough{\lstinline!else!} pertence ao primeiro
  \passthrough{\lstinline!if!} ou ao segundo?
\item
  \textbf{Regra convencional}: \passthrough{\lstinline!else!} liga ao
  \passthrough{\lstinline!if!} mais próximo
\end{itemize}
\end{frame}
...
\begin{frame}{Exemplo de AST para \passthrough{\lstinline!(1+4)*2!}}
\protect\hypertarget{exemplo-de-ast-para-142}{}
\includegraphics[width=0.55\textwidth,height=\textheight]{chapter05/imgs/tree.png}
...
\item
  ...
  \passthrough{\lstinline!HashTable<K,V> x;!} pode ser ambíguo

  \begin{itemize}
  \tightlist
  \item
    Depende se \passthrough{\lstinline!HashTable!} é um tipo ou uma
    variável
  \item
