...
\begin{itemize}
\item
  Apresentar a import\^{a}ncia da análise léxica.
\item
  Apresentar a implementa\c{c}\~{a}o de um analisador léxico ad-hoc.
\end{itemize}
\end{frame}

\hypertarget{anuxe1lise-luxe9xica}{%
\section{Análise Léxica}\label{anuxe1lise-luxe9xica}}
...
\item
  Recebe o código-fonte como \textbf{sequ\^{e}ncia de caracteres}
\item
...
\tightlist
\item
  Remove espa\c{c}os em branco e comentários
\item
...
\item
  Cada token carrega sua \textbf{categoria} (lexema) e posi\c{c}\~{a}o no
  código-fonte
\end{itemize}
\end{frame}
...
data Token = Token
  { pos    :: (Line, Column)  -- posi\c{c}\~{a}o no código-fonte
...
\tightlist
\item
  A posi\c{c}\~{a}o é essencial para \textbf{mensagens de erro} precisas
\item
...
\item
  \(+\): operador de adi\c{c}\~{a}o
\item
  \(*\): operador de multiplica\c{c}\~{a}o
\item
  \((\) e \()\): par\^{e}nteses para agrupamento
\item
  Linguagem simples: express\~{o}es aritméticas básicas
\end{itemize}
\end{frame}
...
\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Token & Construtor & Descri\c{c}\~{a}o \\
\midrule\noalign{}
\endhead
\passthrough{\lstinline!n!} & \passthrough{\lstinline!TNumber Int!} &
número inteiro (carrega o valor) \\
\passthrough{\lstinline!+!} & \passthrough{\lstinline!TPlus!} & operador
de adi\c{c}\~{a}o \\
\passthrough{\lstinline!*!} & \passthrough{\lstinline!TTimes!} &
operador de multiplica\c{c}\~{a}o \\
\bottomrule\noalign{}
\end{longtable}
\end{frame}
...
\midrule\noalign{}
\endhead
\passthrough{\lstinline!(!} & \passthrough{\lstinline!TLParen!} & abre
par\^{e}nteses \\
\passthrough{\lstinline!)!} & \passthrough{\lstinline!TRParen!} & fecha
par\^{e}nteses \\
EOF & \passthrough{\lstinline!TEOF!} & marcador de fim de entrada \\
\bottomrule\noalign{}
\end{longtable}
\end{frame}

\begin{frame}[fragile]{Defini\c{c}\~{a}o do Tipo Lexeme}
...
\item
  \passthrough{\lstinline!TNumber!} carrega o valor inteiro do número
\item
...
\tightlist
\item
  Implementa\c{c}\~{a}o \textbf{manual} e específica para uma linguagem
\item
  Processa caracteres sequencialmente com casamento de padr\~{o}es
\end{itemize}
\end{frame}
...
\tightlist
\item
  N\~{a}o usa geradores automáticos como o Alex
\item
...
\begin{lstlisting}[language=Haskell]
-- Vers\~{a}o simples (sem tratamento de erros):
lexer :: String -> [Token]

-- Vers\~{a}o com tratamento de erros:
...
\tightlist
\item
  \passthrough{\lstinline!Left msg!} indica um \textbf{erro léxico} com
  mensagem \passthrough{\lstinline!msg!}
\item
  \passthrough{\lstinline!Right tokens!} indica \textbf{sucesso} com a
...
\tightlist
\item
  \passthrough{\lstinline!Left a!} representa falha/erro com valor do
  tipo \passthrough{\lstinline!a!}
\item
  \passthrough{\lstinline!Right b!} representa sucesso com valor do tipo
...
\tightlist
\item
  \passthrough{\lstinline!Line!}, \passthrough{\lstinline!Column!}:
  posi\c{c}\~{a}o atual no arquivo de entrada
\item
  \passthrough{\lstinline!String!}: dígitos consecutivos acumulados
  (para montar números)
\item
  \passthrough{\lstinline![Token]!}: tokens reconhecidos até o momento
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Fun\c{c}\~{a}o de Transi\c{c}\~{a}o}
...
transition state@(l, col, t, ts) c
  ...
  | isSpace c  = mkDigits state c   -- espa\c{c}o
...
mkDigits state@(l, col, s, ts) c
  | null s      = -- nenhum dígito acumulado: só atualiza posi\c{c}\~{a}o
      let l'   = if c == '\n' then l + 1 else l
...
\begin{frame}[fragile]{A Fun\c{c}\~{a}o \passthrough{\lstinline!lexer!}
Completa}
...
\begin{frame}[fragile]{Como \passthrough{\lstinline!foldl!} Funciona}
...
Exemplo: \passthrough{\lstinline!sum [1,2,3]!} com
\passthrough{\lstinline!foldl (+) 0!}:
...
\tightlist
\item
  \passthrough{\lstinline!foldl!} percorre a lista \textbf{da esquerda
  para a direita}
\item
...
\begin{frame}[fragile]{Exemplo de Execu\c{c}\~{a}o}
\protect\hypertarget{exemplo-de-execuuxe7uxe3o}{}
Entrada: \passthrough{\lstinline!"2 + 3"!}
...
\passthrough{\lstinline!\_!} &
\passthrough{\lstinline!(1, 3, "", [TNumber 2])!} \\
\passthrough{\lstinline!+!} &
\passthrough{\lstinline!(1, 4, "", [TPlus, TNumber 2])!} \\
\bottomrule\noalign{}
\end{longtable}
\end{frame}
...
\endhead
\passthrough{\lstinline!\_!} &
\passthrough{\lstinline!(1, 5, "", [TPlus, TNumber 2])!} \\
\passthrough{\lstinline!3!} &
\passthrough{\lstinline!(1, 6, "3", [TPlus, TNumber 2])!} \\
EOF & extrai →
\passthrough{\lstinline![TNumber 2, TPlus, TNumber 3]!} \\
\bottomrule\noalign{}
\end{longtable}
\end{frame}
...
\item
  Sem depend\^{e}ncias externas (geradores de código)
\end{itemize}
\end{frame}
...
\item
  ...
  \item
    Como suportar comentários de linha (\passthrough{\lstinline!//!})? E
    comentários em bloco (\passthrough{\lstinline!/* */!})?
  \end{itemize}
\end{itemize}
\end{frame}
...
\item
  ...
  \item
    N\~{a}o há garantia de corre\c{c}\~{a}o: é fácil esquecer um caso
  \end{itemize}
\end{itemize}
\end{frame}
...
\tightlist
\item
  Como distinguir \passthrough{\lstinline!if!} (palavra reservada) de
  \passthrough{\lstinline!ift!} (identificador)?
\item
...
\item
  Solu\c{c}\~{a}o: \textbf{express\~{o}es regulares} + \textbf{aut\^{o}matos finitos}
\end{itemize}
\end{frame}
...
\item
  ...
  tokens}
\item
  Cada token carrega categoria (lexema) e posi\c{c}\~{a}o no fonte
\item
  O tipo \passthrough{\lstinline!Either!} de Haskell permite tratamento
  elegante de erros
\end{itemize}
\end{frame}
...
\item
  As limita\c{c}\~{o}es motivam o uso de \textbf{express\~{o}es regulares e
  aut\^{o}matos}
\end{itemize}
\end{frame}
