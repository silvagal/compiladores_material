...
\begin{frame}[fragile]{Estrutura de um Token}
    ...
    \begin{lstlisting}
        ...
        data Token = Token
        { pos    :: (Line, Column)  -- posic\~ao no c\'odigo-fonte
...
\begin{frame}[fragile]{Definição do Tipo Lexeme}
    ...
    \begin{lstlisting}
        data Lexeme
        = TNumber Int   -- n\'umero com seu valor inteiro
        | TLParen       -- (
        | TRParen       -- )
        | TPlus         -- +
        | TTimes        -- *
...
\begin{frame}[fragile]{Tipo do Analisador}
    ...
    \begin{lstlisting}
        -- Vers\~ao simples (sem tratamento de erros):
        lexer :: String -> [Token]

        -- Vers\~ao com tratamento de erros:
...
\begin{frame}[fragile]{Função de Transição}
    ...
    \begin{lstlisting}
        ...
        | isSpace c  = mkDigits state c   -- espaco
        ...
        | isDigit c  = Right (l, col+1, c:t, ts)  -- acumula d\'igito
...
\begin{frame}[fragile]{Tratamento de Dígitos}
    ...
    \begin{lstlisting}
        ...
        | null s      = -- nenhum d\'igito acumulado: s\'o atualiza posic\~ao
        let l'   = if c == '\n' then l + 1 else l
        col' = if isSpace c then col + 1 else col
        in Right (l', col', s, ts)
        | all isDigit s = -- d\'igitos v\'alidos: cria token TNumber
