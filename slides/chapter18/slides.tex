\subtitle{\bf Aula 18 - Introdu\c{c}\~{a}o}
\begin{frame}[t,plain]
\titlepage
\end{frame}
...
\textbf{Por que só express\~{o}es puras?} \texttt{MEM(e)}
...
Um la\c{c}o \texttt{while} na IRT linearizada tem o padr\~{a}o:
...
depend\^{e}ncias conflitantes entre \texttt{corpo1} e
\texttt{corpo2}.
\end{frame}
...
\item
  \textbf{Alvo estático}: \texttt{NAME(f)}, n\~{a}o um
  ponteiro computado
\item
...
\item
  \texttt{inlineThreshold}: limiar \(\tau\) (padr\~{a}o: 10
  instru\c{c}\~{o}es)
\item
  \texttt{removeDeadFuncs}: elimina fun\c{c}\~{o}es sem
  chamadores após inlining
\item
  M\^{o}nada \texttt{State Int} gera sufixos únicos para
